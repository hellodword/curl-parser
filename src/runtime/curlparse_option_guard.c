...
#include "generated/curlparse_guards_8_20_0.h"
...
static const struct CurlparseOptionGuard *find_option_guard(const char *option)
{
  ...
  for(i = 0; i < sizeof(curlparse_guards_8_20_0) /
                 sizeof(curlparse_guards_8_20_0[0]); ++i) {
    if(strcmp(curlparse_guards_8_20_0[i].option, option) == 0) {
      return &curlparse_guards_8_20_0[i];
    }
  }
...
)
{
  ...
  if(option_disabled_by_profile(profile, event->canonical)) {
    return add_option_not_available(errors, message_option_name(event),
                                    "runtimeProfile.compile disables this option");
  }
