...
def render_options_header(options: list[dict], version: str) -> str:
    version_id = version.replace(".", "_")
    guard = f"CURLPARSE_OPTIONS_{version_id}_H"
    ...
    lines = [
        ...
        "};",
        "",
        f"static const struct CurlparseOptionCatalogEntry curlparse_options_{version_id}[] = {{",
    ]

    for option in options:
        short_name = option["short"] if option["short"] is not None else ""
        lines.append(
            f'  {{"{option["long"]}", "{short_name}", "{option["cmd"]}", "{option["argType"]}"}},'
        )
...
def main() -> int:
    ...
    header_path = workspace_root / "src/generated" / f'curlparse_options_{version.replace(".", "_")}.h'
    header_path.parent.mkdir(parents=True, exist_ok=True)
    header_path.write_text(render_options_header(options, version), encoding="utf-8")
    return 0
